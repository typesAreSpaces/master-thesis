...
z3::expr CDCL_setup::abstract_lit(z3::expr const & lit){
  if(lit.is_not())
...
z3::expr CDCL_setup::abstract_clause(z3::expr const & clause){
...
z3::expr_vector CDCL_setup::abstract_clauses(z3::expr_vector const & clauses){
...
z3::expr_vector CDCL_setup::mk_lits(z3::model const & m){
  z3::expr_vector result(ctx);
...
z3::expr_vector const CDCL_setup::getConflictClauses() const {
  return abstract_conflict_clauses;
}

std::ofstream & CDCL_setup::dimacsLit(std::ofstream & file, z3::expr const & abstract_lit){
...
std::ofstream & CDCL_setup::dimacsClause(std::ofstream & file, z3::expr const & abstract_e){
...
z3::expr CDCL_setup::concretizeAbstraction(int literal){
  assert(literal > 0);
  return concretes.find(PREFIX_PROP_LIT(literal));
}

std::ostream & operator << (std::ostream & os, CDCL_setup const & cdcl){
  return os;
}

CDCL_T::CDCL_T(z3::expr_vector const & formulas, Theory th) :
  CDCL_setup(formulas), 
  theory_solver(ctx, 
      th == EUF ? "QF_UF" : 
      (th == OCT ? "QF_LIA" : 
