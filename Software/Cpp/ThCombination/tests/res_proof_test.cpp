...
int main(){
  ...
  //ResolutionProof test("zchaff ~/booleforce_examples/example2.cnf");

  system("zchaff ~/booleforce_examples/hole6.cnf");
  //system("zchaff ~/booleforce_examples/example2.cnf");
  //system("zchaff ~/booleforce_examples/example1.cnf");

  std::string current_dir = exec("pwd");
  std::ifstream resolve_trace((current_dir + "/resolve_trace"), std::fstream::in);

  std::string line, proof_kind, aux_symbol;
  while(std::getline(resolve_trace, line)){
