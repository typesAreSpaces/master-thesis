...
class CDCL_setup { 
  ...
  CDCL_setup(z3::expr_vector const &);
  ...
  friend std::ostream & operator << (std::ostream &, CDCL_setup const &);
};

class CDCL_T : public CDCL_setup {

  z3::solver theory_solver;
  void loop();

  public:
  CDCL_T(z3::expr_vector const &, Theory);
};
