...
inequalityListToLatex :: Int -> [Inequality] -> String
inequalityListToLatex c []       = ""
inequalityListToLatex c (x : xs) = show x ++ " \\leq b_" ++ show c ++ ", " ++ inequalityListToLatex (c + 1) xs
...
main :: IO ()
main = putStrLn . (\x -> "$" ++ x ++ "$") . (inequalityListToLatex 0) . qs $ test
